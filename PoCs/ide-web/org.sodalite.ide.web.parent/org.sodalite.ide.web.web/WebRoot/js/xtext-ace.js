...
define('xtext/ServiceBuilder',[
	...
	ServiceBuilder.prototype.createServices = function() {
		...
		services.successListeners = [function(serviceType, requestData) {
			if (serviceType == "save"){
		...
		services.errorListeners = [function(serviceType, severity, message, requestData) {
			var myRegexp = new RegExp("<title[^>]*>(.*?)<\/title>");
			var match = myRegexp.exec(requestData.responseText);
			if (serviceType == "save")
				serviceType = "Save AADM into the KB";
			if (match != null)
				message = match[1]
			if (options.showErrorDialogs)
				window.alert('SODALITE IDE: \'' + serviceType + '\' failed: ' + message);
			else
				console.log('SODALITE IDE:  \'' + serviceType + '\' failed: ' + message);
		}];
	}
