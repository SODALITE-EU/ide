...
public class DeregisterAlertsHandler implements IHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		try {
			if (PlatformUI.getWorkbench().saveAllEditors(true)) { // Ask to save model before continue
				AlertingBackendProxy.startDeregisteringAlertingRulesWizard(event);
			}
		} catch (Exception ex) {
			MessageDialog.openError(parent, "Deregister alerting rules in monitoring",
